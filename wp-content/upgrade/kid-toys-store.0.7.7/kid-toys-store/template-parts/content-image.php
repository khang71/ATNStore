...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="postbox mdallpostimage p-2 mb-4">
    <div class="row">
      <?php if(has_post_thumbnail() && get_theme_mod( 'kid_toys_store_feature_image_hide',true) != '') { ?>
        <div class="box-image col-lg-5 col-md-5">
          <?php the_post_thumbnail();  ?>
        </div>
      <?php } ?>
      <div class="new-text <?php 
        if(has_post_thumbnail() && get_theme_mod( 'kid_toys_store_feature_image_hide',true) != '') { ?>col-lg-7 col-md-7"<?php } else { ?>col-lg-12"<?php } ?>>
        <div class="box-content">
          ...
          <?php if( get_theme_mod( 'kid_toys_store_date_hide',true) != '' || get_theme_mod( 'kid_toys_store_comment_hide',true) != '' || get_theme_mod( 'kid_toys_store_author_hide',true) != '' || get_theme_mod( 'kid_toys_store_time_hide',true) != '') { ?>
            <div class="metabox p-1 mb-2">
              <?php if( get_theme_mod( 'kid_toys_store_date_hide',true) != '') { ?>
                <span class="entry-date px-1"><i class="far fa-calendar-alt me-1"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span>
              <?php } ?>

              <?php if( get_theme_mod( 'kid_toys_store_author_hide',true) != '') { ?>
                <span class="entry-author px-1"><i class="fas fa-user me-1"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php esc_html(the_author()); ?><span class="screen-reader-text"><?php esc_html(the_author()); ?></span></a></span>
              <?php } ?>

              <?php if( get_theme_mod( 'kid_toys_store_comment_hide',true) != '') { ?>
                <span class="entry-comments px-1"><i class="fa fa-comments me-1" aria-hidden="true"></i><?php comments_number( __('0 Comment', 'kid-toys-store'), __('0 Comments', 'kid-toys-store'), __('% Comments', 'kid-toys-store') ); ?> </span>
              <?php } ?>

              <?php if( get_theme_mod( 'kid_toys_store_time_hide',false) != '') { ?>
